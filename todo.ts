...
const mainFunction = async () => {
    while (condition) {
        ...
        if (mainTask.select === "Add") { await addtask() }
        if (mainTask.select === "View Task") { await view_task() }
        if (mainTask.select === "Update") { await taskUpdate() }
        if (mainTask.select === "Delete") { await Delete() }
